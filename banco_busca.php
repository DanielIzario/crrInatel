<?php
	...
	// Consulta o banco
	$sql = 'SELECT COD_IBGE, NOME_MUNICIP, UF, REGIAO,  '. $from .' FROM municipios_2015 WHERE COD_IBGE LIKE ? OR NOME_MUNICIP LIKE ? OR UF LIKE ? OR REGIAO LIKE ? OR '. $from .' LIKE ?';
	$values = array($value, $value, $value, $value, $value); 	// Sera substituido pelas interrogacoes na querry
	...
	while ( $row = $db->fetch() ) {
	  	echo $row['COD_IBGE'] . '#' . $row['NOME_MUNICIP'] . '#' . $row['UF']. '#' . $row['REGIAO'] . '#' . $row[($from)] . '#';
	}
?>